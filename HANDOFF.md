# Agent handoff — Export Img (`v1.0.8`)
...
**Date:** 2026-07-23  
**Branch:** `v1.0.8` (cut from `main` @ `d81dfc7` / release **1.0.7**)  
**Target ship:** `1.0.8` — bump already applied in `package.json` / `manifest.json` / `versions.json`
...
| Do **not** use Tag `Version_*` or `v1.0.8` as release tag | Conflicts with branch / store |
...
1. Finish work on `v1.0.8`; keep versions at `1.0.8`.
2. `git checkout main && git pull && git merge --squash v1.0.8`
3. Commit: `release(1.0.8): <summary>`
4. `git push origin main` → [`.github/workflows/release.yml`](.github/workflows/release.yml) builds, attests `main.js`/`styles.css`, and creates tag **`1.0.8`** + assets.

If tag already exists, workflow skips. Delete GitHub Release + tag to rebuild.

Latest public release on `main`: **1.0.7** (`d81dfc7`).

---

## Shipped in 1.0.7 (context)

- Studio preview: keep pan/zoom across updates — **only** first open + double-click/tap apply 94%/3% fit (refresh does not reset)
- Author bar: host CSS forces sizer children into normal flow; `layoutAuthorBar` extends sizer for overflow media
- Watermark: canvas stamp with tile-step / canvas-pixel caps; lighter defaults (`Watermark`, 22px, 400 weight, 30°, 15%)
- Refresh: preview-pane `mod-cta`「更新预览」; Copy/Save refresh stale host first (`busyRef`)
- Watermark/author toggles: session-only; migrate v4 clears persisted enable/show
- `autoRerenderPreview`: desktop on / mobile off; migrate v5 fills only when key missing
- Preview progress bar; settings restore-defaults + live `%` / `°` sliders
